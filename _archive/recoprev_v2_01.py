...
def check_logger_file(filename):
    global GlbStatus
    if not(path.exists(filename)):
        print(filename + " does not exist.")
        with open(filename, mode='w') as logger_file:
            header = ['Timestamp', 'SmileyEvent']
            writer = csv.DictWriter(logger_file, fieldnames=header)
            writer.writeheader()
        GlbStatus = filename + " created."
        logger_file.close()
    else:
        print(filename + " exits.")
        GlbStatus = filename + " exists."
