...
GlbStatus = 'waiting for input -- v3.00'
...
def time_between_inDays(dti1, dti2):
    dtim1 = datetime.strptime(dti1, "%Y-%m-%d %H:%M:%S")
    dtim2 = datetime.strptime(dti2, "%Y-%m-%d %H:%M:%S")
    return abs((dtim2 - dtim1).days)-1

def time_between_inSeconds(sti1, sti2):
    stim1 = datetime.strptime(sti1, "%Y-%m-%d %H:%M:%S")
    stim2 = datetime.strptime(sti2, "%Y-%m-%d %H:%M:%S")
    return abs((stim2 - stim1).total_seconds())

def display_time():
    clock_time = tm.strftime('%H:%M:%S')
    cst = tm.strftime('%Y-%m-%d %H:%M:%S')
    ...
    timetozero_label['text'] = str(time_between_inDays(cst, myPointZero))
